...
import { buildFrontendPrompt } from "../prompts/frontendPrompt";
...
export async function runFrontendAgent(
  ...
  const systemPrompt =
    "You are GEMINI role: an expert frontend engineer shipping polished React + Tailwind UI fast.";
  const userPrompt = buildFrontendPrompt(input, plannerOutput);
